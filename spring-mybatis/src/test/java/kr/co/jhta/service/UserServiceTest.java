 ...
 * 		- 테스트 대상 객체를 정의되어 있는 스프링 빈 설정파일의 경로를 지정하는 어노테이션이다.
 * @RunWith
 ...
 * @Transactional
 * 		- 단위테스트 클래스에 @Transactional 어노테이션(선언적 트랜잭션 처리)을 부착하면
 * 		    테스트 작업 과정에서 수행한 INSERT/IPDATE/DELETE 작업을 Database에 반영시키지 않고
 ...
 *		- 테스트를 반복 수행할 수 있게 된다.
 *		- 
 *		- 2개이상의 기능을 사용하는 곳에서 트랜젹션기능을 사용한다. 그래서 dao가 아니라 service에서 사용해야한다.
 */
...
public class UserServiceTest {
	...
	public void testAddNewUser() {
		User user = new User("seoul", "서울", "zxcv1234", "seoul@gamail.com");
		userService.addNewUser(user);
		
		User savedUser = userDao.getUserById(user.getId());
		assertNotNull(savedUser);
		assertEquals("서울", savedUser.getName());
	}
	...
	public void testLogin() {
		User user = userService.login("lee", "zxcv1234");
